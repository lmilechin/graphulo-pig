...
public abstract class AbstractGraphuloStorage extends AbstractAccumuloStorage implements
     ...
     * @param opts
     */
    protected void extractArgs(CommandLine cli, GraphuloStorageOptions opts)
            throws IOException {
        ...
        this.start = cli.getOptionValue(
                GraphuloStorageOptions.START_ROW_OPTION.getOpt(), null);
        this.end = cli.getOptionValue(
                GraphuloStorageOptions.END_ROW_OPTION.getOpt(), null);

        if (cli.hasOption(GraphuloStorageOptions.MAX_LATENCY_OPTION.getOpt())) {
            this.maxLatency = opts.getInt(cli,
                    GraphuloStorageOptions.MAX_LATENCY_OPTION);
        }

        if (cli.hasOption(GraphuloStorageOptions.WRITE_THREADS_OPTION.getOpt())) {
            this.maxWriteThreads = opts.getInt(cli,
                    GraphuloStorageOptions.WRITE_THREADS_OPTION);
        }

        if (cli.hasOption(GraphuloStorageOptions.MUTATION_BUFFER_SIZE_OPTION
                .getOpt())) {
            this.maxMutationBufferSize = opts.getLong(cli,
                    GraphuloStorageOptions.MUTATION_BUFFER_SIZE_OPTION);
        }
        ...
        if (cli.hasOption(GraphuloStorageOptions.COLUMN_SEPARATOR_OPTION
                .getOpt())) {
            columnSeparator = cli
                    .getOptionValue(GraphuloStorageOptions.COLUMN_SEPARATOR_OPTION
                            .getOpt());
        }

        if (cli.hasOption(GraphuloStorageOptions.COLUMN_IGNORE_WHITESPACE_OPTION
                .getOpt())) {
            String value = cli
                    .getOptionValue(GraphuloStorageOptions.COLUMN_IGNORE_WHITESPACE_OPTION
                            .getOpt());
            if ("false".equalsIgnoreCase(value)) {
                ...
                log.warn("Ignoring unknown value for "
                        + GraphuloStorageOptions.COLUMN_IGNORE_WHITESPACE_OPTION
                                .getOpt() + ": " + value);
            }
        }
    }
